...
if ( empty( $pmpro_invoice ) ) {
	$pmpro_msg = __( 'אירעה שגיאה בעת אחזור ההזמנה שלך. אנא צור קשר עם בעל האתר.', 'paid-memberships-pro' );
		...
		<?php
			...
			if ( ! empty( $pmpro_invoice ) ) {
				...
				if ( 'success' != $pmpro_invoice->status ) {
					$confirmation_message = '<p>' . sprintf(__('תודה על המנוי שלך ל-%1$s. המנוי שלך ל-%2$s יופעל לאחר השלמת התשלום.', 'paid-memberships-pro' ), get_bloginfo("name"), $pmpro_invoice->membership_level->name) . '</p>';
				} else {
					$confirmation_message = '<p>' . sprintf(__('תודה על המנוי שלך ל-%s. המנוי שלך ל-%s פעיל כעת.', 'paid-memberships-pro' ), get_bloginfo("name"), $pmpro_invoice->membership_level->name) . '</p>';
				}
				...
				if ( ! pmpro_isLevelFree( $pmpro_invoice->membership_level ) ) {
					$confirmation_message .= '<p>' . sprintf( __( 'להלן פרטים על חשבון המנוי שלך וקבלה על ההזמנה הראשונית שלך. דוא"ל ברוך הבא עם עותק של ההזמנה הראשונית שלך נשלח ל-%s.', 'paid-memberships-pro' ), $pmpro_invoice->user->user_email ) . '</p>';
				} else {
					$confirmation_message .= '<p>' . sprintf( __( 'להלן פרטים על חשבון המנוי שלך. דוא"ל ברוך הבא נשלח ל-%s.', 'paid-memberships-pro' ), $pmpro_invoice->user->user_email ) . '</p>';
				}
				...
				if ( 'success' != $pmpro_invoice->status ) {
					...
					<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_message pmpro_alert' ) ); ?>">
						<?php
							 ...
							 */
							echo wp_kses_post( apply_filters( 'pmpro_confirmation_payment_incomplete_message', __( 'אנו ממתינים להשלמת התשלום שלך.', 'paid-memberships-pro' ), $pmpro_invoice ) );
						?>
					</div> <!-- pmpro_message -->
					<?php
				}
					...
					<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card' ) ); ?>">
						<h3 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_title pmpro_font-large pmpro_heading-with-avatar' ) ); ?>">
							...
							<?php
								...
								printf( esc_html__( 'ברוך הבא, %s', 'paid-memberships-pro' ), esc_html( $pmpro_invoice->user->display_name ) );
							?>
						</h3>
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
							<ul class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list pmpro_list-plain' ) ); ?>">
								<li class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list_item' ) ); ?>"><strong><?php esc_html_e( 'שם משתמש', 'paid-memberships-pro' ); ?>:</strong> <?php echo esc_html( $pmpro_invoice->user->user_login ); ?></li>
								<li class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list_item' ) ); ?>"><strong><?php esc_html_e( 'דוא"ל', 'paid-memberships-pro' );?>:</strong> <?php echo esc_html( $pmpro_invoice->user->user_email ); ?></li>
								<li class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list_item' ) ); ?>">
									<strong><?php esc_html_e('רמת מנוי', 'paid-memberships-pro' );?>:</strong>
									<?php echo empty( $pmpro_invoice->membership_level ) ? esc_html__( 'ממתין', 'paid-memberships-pro' ) : esc_html( $pmpro_invoice->membership_level->name ); ?>
								</li>
								<?php
									/**
