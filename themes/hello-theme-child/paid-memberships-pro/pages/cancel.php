	...
	<section id="pmpro_cancel" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_section', 'pmpro_cancel' ) ); ?>">
		...
		if ( empty( $_REQUEST['confirm'] ) && isset( $_REQUEST['levelstocancel'] ) ) {
			...
			if( $_REQUEST['levelstocancel'] !== 'all') {
				...
				$are_you_sure  = sprintf( _n( 'האם אתה בטוח שברצונך לבטל את המנוי שלך ל-%s?', 'האם אתה בטוח שברצונך לבטל את המנויים שלך ל-%s?', count( $old_level_ids ), 'paid-memberships-pro' ), pmpro_implodeToEnglish( $level_names ) );
				$cancel_memberships_text = _n( 'כן, בטל מנוי זה', 'כן, בטל מנויים אלו', count( $old_level_ids ), 'paid-memberships-pro' );
				$keep_memberships_text = _n( 'לא, שמור על מנוי זה', 'לא, שמור על מנויים אלו', count( $old_level_ids ), 'paid-memberships-pro' );
			} else {
				// All levels are being cancelled.
				$old_level_ids = wp_list_pluck( $user_levels, 'id' );

				// Build messages.
				$are_you_sure = _n( 'האם אתה בטוח שברצונך לבטל את המנוי שלך?', 'האם אתה בטוח שברצונך לבטל את כל המנויים שלך?', count( $user_levels ), 'paid-memberships-pro' );
				$cancel_memberships_text = _n( 'כן, בטל את המנוי שלי', 'כן, בטל את כל המנויים שלי', count( $user_levels ), 'paid-memberships-pro' );
				$keep_memberships_text = _n( 'לא, שמור על המנוי שלי', 'לא, שמור על כל המנויים שלי', count( $user_levels ), 'paid-memberships-pro' );
			}
			...
			if ( count( $old_level_ids ) <= 1 ) {
				if ( ! empty( $subscriptions ) ) {
					...
					$cancellation_behavior_text = sprintf( __( 'המנוי שלך יבוטל. לא תחויב שוב. המנוי שלך יישאר פעיל עד %s.', 'paid-memberships-pro' ), $subscriptions[0]->get_next_payment_date( get_option( 'date_format' ) ) );
				} else {
					// No subscription. Show a generic message.
					$cancellation_behavior_text = __( 'המנוי שלך יבוטל מיד.', 'paid-memberships-pro' );
				}
			} else {
				if ( ! empty( $subscriptions ) ) {
					// There is a subscription. Show a generic message.
					$cancellation_behavior_text = __( 'חלק מהמנויים שאתה מבטל כוללים מנוי חוזר. מנויים אלו יבוטלו, לא תחויב שוב, והמנויים הקשורים יישארו פעילים עד סוף תקופת התשלום הנוכחית.', 'paid-memberships-pro' );
				} else {
					// No subscription. Show a generic message.
					$cancellation_behavior_text = __( 'המנויים שלך יבוטלו מיד.', 'paid-memberships-pro' );
				}
			}
			...
			<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card' ) ); ?>">
				<h2 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_title pmpro_font-large' ) ); ?>"><?php esc_html_e( 'המנויים שלי', 'paid-memberships-pro' ); ?></h2>
				<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
					<p><?php esc_html_e( 'יש לך את המנויים הפעילים הבאים.', 'paid-memberships-pro' ); ?></p>
					...
					<table class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table pmpro_table_cancel', 'pmpro_table_cancel' ) ); ?>">
						<thead>
							<tr>
								<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_cancel-level' ) ); ?>"><?php esc_html_e("רמה", 'paid-memberships-pro' );?></th>
								<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_cancel-expiration' ) ); ?>"><?php esc_html_e("תפוגה", 'paid-memberships-pro' ); ?></th>
								<th></th>
							</tr>
						</thead>
						<tbody>
							<?php
								...
								?>
								<tr>
									<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_cancel-level' ) ); ?>" data-title="<?php esc_attr_e( 'רמה', 'paid-memberships-pro' ); ?>">
										<?php echo esc_html( $level->name );?>
									</th>
									<td class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_cancel-expiration' ) ); ?>" data-title="<?php esc_attr_e( 'תפוגה', 'paid-memberships-pro' ); ?>">
									<?php
									...
									<td class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_cancel-action' ) ); ?>">
										<a href="<?php echo esc_url( pmpro_url( "cancel", "?levelstocancel=" . $level->id ) ) ?>"><?php esc_html_e("בטל", 'paid-memberships-pro' );?></a>
									</td>
								</tr>
								<?php
								}
				...
				<?php
					...
					if ( count( $user_levels ) > 1 ) {
						...
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_actions' ) ); ?>">
							<a href="<?php echo esc_url( pmpro_url( "cancel", "?levelstocancel=all" ) ); ?>"><?php esc_html_e("בטל את כל המנויים", 'paid-memberships-pro' );?></a>
			...
			<?php
				if ( empty( $user_levels ) ) {
					...
					?>
					<span class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_cancel_return_home pmpro_actions_nav-right', 'pmpro_cancel_return_home' ) ); ?>"><a href="<?php echo esc_url( get_home_url() )?>"><?php esc_html_e( 'צפה בדף הבית &rarr;', 'paid-memberships-pro' ); ?></a></span>
					<?php
				} else {
