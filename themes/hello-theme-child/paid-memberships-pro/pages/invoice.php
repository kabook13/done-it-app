...
<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro' ) ); ?>">
	...
	if ( $pmpro_invoice ) {
				...
				<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
						...
						<ul class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list pmpro_list-plain pmpro_list-with-labels pmpro_cols-2' ) ); ?>">
							<?php
								...
								$pmpro_order_single_meta['order_date'] = array(
									'label' => __( 'תאריך הזמנה', 'paid-memberships-pro' ),
								...
								if ( $pmpro_invoice->accountnumber ) {
									$pmpro_order_single_meta['payment_method'] = array(
										'label' => __( 'שיטת תשלום', 'paid-memberships-pro' ),
								...
								$pmpro_order_single_meta['pay_to'] = array(
									'label' => __( 'שלם ל-', 'paid-memberships-pro' ),
									'value' => $pay_to,
								);

								// Bill to.
								$pmpro_order_single_meta['bill_to']['label'] = __( 'חייב ל-', 'paid-memberships-pro' );
					...
					<?php
						...
						if ( empty( $pmpro_price_parts ) ) {
							...
							$pmpro_price_parts = array( 'total' => array( 'label' => __( 'סה"כ', 'paid-memberships-pro' ), 'value' => $pmpro_invoice->get_formatted_total() ) );
						}

						// If the order is refunded, add to price parts.
						if ( $pmpro_invoice->status == 'refunded' ) {
							$pmpro_price_parts['refunded']['label'] = __( 'הוזל', 'paid-memberships-pro' );
					...
					<div id="pmpro_order_single-items">
							...
							if ( (float)$pmpro_invoice->total > 0 && in_array( $pmpro_invoice->status, array( '', 'success', 'cancelled' ) ) ) {
								...
								<h3 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_font-large' ) ); ?>">
									<?php
										echo esc_html(
											sprintf(
												__( '%1$s שולם ב-%2$s', 'paid-memberships-pro' ),
						...
						<table class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table' ) ); ?>">
							<thead>
								<tr>
									<th><?php esc_html_e( 'תיאור', 'paid-memberships-pro' ); ?></th>
									<th><?php esc_html_e( 'סכום', 'paid-memberships-pro' ); ?></th>
								</tr>
							</thead>
							<tbody>
								<tr>
									<th data-title="<?php esc_attr_e( 'תיאור', 'paid-memberships-pro' ); ?>">
										<?php
											echo esc_html(
												sprintf(
													// translators: 1: level name, 2: order code
													__( '%1$s עבור הזמנה #%2$s', 'paid-memberships-pro' ),
											...
											if ( ! empty( $pmpro_invoice->billing->name ) ) {
												echo '<p>' . esc_html(
													sprintf(
														...
														__( 'חשבון: %1$s (%2$s)', 'paid-memberships-pro' ),
										...
										<?php
											...
											if ( ! empty( $subscription_period_end ) && $subscription_period_end !== $order_date ) {
												...
												<p class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_font-small' ) ); ?>">
													<?php echo esc_html( sprintf( __( '%1$s עד %2$s', 'paid-memberships-pro' ), $order_date, $subscription_period_end ) ); ?>
												</p>
												<?php
											}
										?>
									</th>
									<td data-title="<?php esc_attr_e( 'סכום', 'paid-memberships-pro' ); ?>">
						...
						<?php if ( $pmpro_invoice->getDiscountCode() ) { ?>
							<ul class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list pmpro_list-plain pmpro_list-with-labels pmpro_cols-2' ) ); ?>">
								<li class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list_item' ) ); ?>">
									<span class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_list_item_label' ) ); ?>"><?php esc_html_e('קוד הנחה', 'paid-memberships-pro' ); ?></span>
		...
		<section id="pmpro_order_list" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_section', 'pmpro_order_list' ) ); ?>">
			<h2 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_section_title pmpro_font-x-large' ) ); ?>">
				<?php esc_html_e( 'היסטוריית הזמנות', 'paid-memberships-pro' ); ?>
			</h2>
			<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card' ) ); ?>">
				<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
				<?php
					if ( $orders ) {
						...
						<table class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table pmpro_table_orders', 'pmpro_table_orders' ) ); ?>">
							<thead>
								<tr>
									<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-date' ) ); ?>"><?php esc_html_e( 'תאריך', 'paid-memberships-pro' ); ?></th>
									<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-level' ) ); ?>"><?php esc_html_e( 'רמה', 'paid-memberships-pro' ); ?></th>
									<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-amount' ) ); ?>"><?php esc_html_e( 'סה"כ', 'paid-memberships-pro' ); ?></th>
									<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-status' ) ); ?>"><?php esc_html_e( 'סטטוס', 'paid-memberships-pro'); ?></th>
								</tr>
							</thead>
							<tbody>
							<?php
								foreach( $orders as $order ) {
									...
									if ( in_array( $order->status, array( '', 'success', 'cancelled' ) ) ) {
										$display_status = esc_html__( 'שולם', 'paid-memberships-pro' );
										$tag_style = 'success';
									} elseif ( $order->status == 'pending' ) {
										// Some Add Ons set status to pending.
										$display_status = esc_html__( 'ממתין', 'paid-memberships-pro' );
										$tag_style = 'alert';
									} elseif ( $order->status == 'refunded' ) {
										$display_status = esc_html__( 'הוחזר', 'paid-memberships-pro' );
										$tag_style = 'error';
									}
									?>
									<tr id="pmpro_table_order-<?php echo esc_attr( $order->code ); ?>">
										<th class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-date' ) ); ?>" data-title="<?php esc_attr_e( 'תאריך', 'paid-memberships-pro' ); ?>"><a href="<?php echo esc_url( pmpro_url( "invoice", "?invoice=" . $order->code ) ) ?>"><?php echo esc_html( date_i18n(get_option("date_format"), $order->getTimestamp()) )?></a></th>
										<td class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-level' ) ); ?>" data-title="<?php esc_attr_e( 'רמה', 'paid-memberships-pro' ); ?>"><?php if(!empty($order->membership_level)) echo esc_html( $order->membership_level->name ); else echo esc_html__("N/A", 'paid-memberships-pro' );?></td>
										<td class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-amount' ) ); ?>" data-title="<?php esc_attr_e( 'סה"כ', 'paid-memberships-pro' ); ?>"><?php
											//phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
											echo pmpro_escape_price( $order->get_formatted_total() ); ?></td>
										<td class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_table_order-status' ) ); ?>" data-title="<?php esc_attr_e( 'סטטוס', 'paid-memberships-pro' ); ?>">
					...
					<?php } else {
						?>
						<p><?php esc_html_e( 'לא נמצאו הזמנות.', 'paid-memberships-pro' ); ?></p>
						<?php
					}
				?>
				</div> <!-- end pmpro_card_content -->
	...
	<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_actions_nav' ) ); ?>">
		<span class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_actions_nav-right' ) ); ?>"><a href="<?php echo esc_url( pmpro_url( "account" ) ) ?>"><?php esc_html_e('צפה בחשבון המנוי שלך &rarr;', 'paid-memberships-pro' );?></a></span>
		<?php if ( $pmpro_invoice ) { ?>
			<span class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_actions_nav-left' ) ); ?>"><a href="<?php echo esc_url( pmpro_url( "invoice" ) ) ?>"><?php esc_html_e('&larr; צפה בכל ההזמנות', 'paid-memberships-pro' );?></a></span>
		<?php } ?>
