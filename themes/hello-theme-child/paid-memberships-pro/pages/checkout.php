...
<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro' ) ); ?>">
	...
	<section id="pmpro_level-<?php echo intval( $pmpro_level->id ); ?>" class="<?php echo esc_attr( pmpro_get_element_class( $pmpro_checkout_gateway_class, 'pmpro_level-' . $pmpro_level->id ) ); ?>">
		...
		<form id="pmpro_form" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form' ) ); ?>" action="<?php if(!empty($_REQUEST['review'])) echo esc_url( pmpro_url("checkout", "?pmpro_level=" . $pmpro_level->id ) ); ?>" method="post">
			...
			<?php if ( $pmpro_review ) { ?>
				<p><?php echo wp_kses( __( 'כמעט סיימנו! כדאי לעבור רגע על סוג המנוי והמחיר שמופיע למטה, ואז לחץ על כפתור <strong> השלם הזמנה </strong> כדי לסיים את ההזמנה.', 'paid-memberships-pro' ), array( 'strong' => array() ) ); ?></p>
			<?php } ?>
			...
			<?php
				...
				<div id="pmpro_pricing_fields" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card', 'pmpro_pricing_fields' ) ); ?>">
					...
					<h2 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_title pmpro_font-large' ) ); ?>"><?php esc_html_e( 'פרטי מנוי', 'paid-memberships-pro' ); ?></h2>
					...
					<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
						...
						<p class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_level_name_text' ) );?>">
							...
							$level_text = sprintf( esc_html__('בחרת במנוי %s .', 'paid-memberships-pro' ), '<strong>' . esc_html( $pmpro_level->name ) . '</strong>' );
							...
							$level_text = preg_replace('/סוג\s*המנוי\s*[:：]\s*[\.。]\s*/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*המנוי\s*[\.。]\s*/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*המנוי\s*[:：]\s*/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*מנוי\s*[:：]\s*[\.。]\s*/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*מנוי\s*[\.。]\s*/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*מנוי\s*[:：]\s*/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*מנוי\s*[\.。]\s*[\.。]/iu', '', $level_text);
							$level_text = preg_replace('/סוג\s*מנוי\s*[:：]\s*[\.。]\s*[\.。]/iu', '', $level_text);
							// תיקון "בחרת ב X סוג מנוי:."
							$level_text = preg_replace('/בחרת ב\s+([^<]*?)\s+סוג מנוי:\s*\./i', 'בחרת במנוי $1.', $level_text);
							$level_text = preg_replace('/בחרת ב\s+([^<]*?)\s+סוג מנוי\./i', 'בחרת במנוי $1.', $level_text);
							$level_text = preg_replace('/בחרת ב\s+([^<]*?)\s+סוג מנוי:/i', 'בחרת במנוי $1.', $level_text);
							// תיקון "בחרת ב X ." (ללא "סוג מנוי")
							$level_text = preg_replace('/בחרת ב\s+([^<]*?)\s*\.\s*</i', 'בחרת במנוי $1.<', $level_text);
							// תיקון נקודה כפולה אחרי "מנוי"
							$level_text = preg_replace('/מנוי\s*:\s*\./i', 'מנוי.', $level_text);
							$level_text = preg_replace('/מנוי\s*\.\s*\./i', 'מנוי.', $level_text);
							// תיקון רווחים מיותרים
							$level_text = preg_replace('/ב\s+סוג/i', 'בסוג', $level_text);
							echo $level_text;
							...
							if ( ! empty( $group ) && empty( $group->allow_multiple_selections ) ) {
								...
								if ( ! empty( $levels ) ) {
									foreach ( $levels as $level ) {
										...
										printf( ' ' . esc_html__( 'המנוי הנוכחי שלך %s יתחלף ברכישה זו.', 'paid-memberships-pro' ), '<strong>' . esc_html( $level->name ) . '</strong>' );
									}
								}
							}
							?>
						...
						<div id="pmpro_level_cost">
							<?php if($discount_code && pmpro_checkDiscountCode($discount_code)) { ?>
								<?php
									...
									echo sprintf( esc_html__( 'הקוד %s הוחל על ההזמנה שלך.', 'paid-memberships-pro' ), '<span class="' . esc_attr( pmpro_get_element_class( "pmpro_tag pmpro_tag-discount-code", "pmpro_tag-discount-code" ) ) . '">' . esc_html( $discount_code ) . '</span>' );
							...
							<?php
								...
								if ( ! empty( $level_cost_text ) ) {
									...
									// הסרת המילה "עכשיו"
									$level_cost_text = preg_replace('/\s+עכשיו\s*/i', '', $level_cost_text);
									// הסרת "סוג מנוי:." / "סוג מנוי." / "סוג מנוי:" מכל המקומות - כל הווריאציות
									// תופס גם עם רווחים, נקודות, נקודתיים, HTML entities וכו'
									// תיקון: תופס גם "סוג המנוי" (עם ה' הידיעה)
									$level_cost_text = preg_replace('/סוג\s*המנוי\s*[:：]\s*[\.。]\s*/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*המנוי\s*[\.。]\s*/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*המנוי\s*[:：]\s*/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*מנוי\s*[:：]\s*[\.。]\s*/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*מנוי\s*[\.。]\s*/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*מנוי\s*[:：]\s*/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*מנוי\s*[\.。]\s*[\.。]/iu', '', $level_cost_text);
									$level_cost_text = preg_replace('/סוג\s*מנוי\s*[:：]\s*[\.。]\s*[\.。]/iu', '', $level_cost_text);
									// תיקון נקודה כפולה אחרי "מנוי" - כל וריאציה אפשרית
									$level_cost_text = preg_replace('/מנוי\s*:\s*\./i', 'מנוי.', $level_cost_text);
									$level_cost_text = preg_replace('/מנוי\s*\.\s*\./i', 'מנוי.', $level_cost_text);
									$level_cost_text = preg_replace('/מנוי\s*\.\s*\.\s*\./i', 'מנוי.', $level_cost_text);
									$level_cost_text = preg_replace('/מנוי\s*:\s*\.\s*\./i', 'מנוי.', $level_cost_text);
									// תיקון "המחיר עבור חשבון הוא"
									$level_cost_text = preg_replace('/המחיר עבור חשבון הוא\s*\.\s*עכשיו/i', 'המחיר עבור המנוי הוא', $level_cost_text);
									$level_cost_text = preg_replace('/המחיר עבור חשבון הוא/i', 'המחיר עבור המנוי הוא', $level_cost_text);
									// תיקון רווחים מיותרים בין אותיות למילים
									$level_cost_text = preg_replace('/ב\s+סוג/i', 'בסוג', $level_cost_text);
									$level_cost_text = preg_replace('/ה\s+מחיר/i', 'המחיר', $level_cost_text);
									$level_cost_text = preg_replace('/ל\s+חודש/i', 'לחודש', $level_cost_text);
									$level_cost_text = preg_replace('/ל\s+שנה/i', 'לשנה', $level_cost_text);
								?>
					...
					<?php if ( $pmpro_show_discount_code ) { ?>
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_actions' ) ); ?>">
							<?php if($discount_code && !$pmpro_review) { ?>
								<span id="other_discount_code_p"><button type="button" id="other_discount_code_toggle"><?php esc_html_e('לחץ כאן להחלפת קוד קופון', 'paid-memberships-pro' );?></button></span>
							<?php } elseif(!$pmpro_review) { ?>
								<span id="other_discount_code_p"><?php esc_html_e('האם יש לך קוד קופון?', 'paid-memberships-pro' );?> <button type="button" id="other_discount_code_toggle"><?php esc_html_e('לחץ כאן להזין קוד קופון', 'paid-memberships-pro' );?></button></span>
							<?php } elseif($pmpro_review && $discount_code) { ?>
								<span><strong><?php esc_html_e('קוד קופון', 'paid-memberships-pro' );?>:</strong> <?php echo esc_html( $discount_code ); ?></span>
							<?php } ?>
							<div id="other_discount_code_fields" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text' ) ); ?>" style="display: none;">
								<label for="pmpro_other_discount_code" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('קוד קופון', 'paid-memberships-pro' );?></label>
								<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fields-inline' ) ); ?>">
									...
									<input aria-label="<?php esc_html_e( 'החל קוד קופון', 'paid-memberships-pro' ); ?>" type="button" name="other_discount_code_button" id="other_discount_code_button" value="<?php esc_attr_e('החל', 'paid-memberships-pro' );?>" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_btn pmpro_btn-submit-discount-code', 'other_discount_code_button' ) ); ?>" />
								</div>
							</div>
					...
					<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card' ) ); ?>">
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
							<legend class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_legend' ) ); ?>">
								<h2 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_heading pmpro_font-large' ) ); ?>"><?php esc_html_e( 'פרטי משתמש', 'paid-memberships-pro' ); ?></h2>
							</legend>
							<?php if ( ! $skip_account_fields ) { ?>
								<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fields' ) ); ?>">
									...
									<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-username', 'pmpro_form_field-username' ) ); ?>">
										<label for="username" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('שם משתמש', 'paid-memberships-pro' );?></label>
									...
									<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-password' ) ); ?>">
										<label for="password" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>">
											<?php esc_html_e( 'סיסמה', 'paid-memberships-pro' );?>
										</label>
										...
										<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field-password-toggle' ) ); ?>">
											<button type="button" class="pmpro_btn pmpro_btn-plain pmpro_btn-password-toggle hide-if-no-js" data-toggle="0">
												<span class="pmpro_icon pmpro_icon-eye" aria-hidden="true"><svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--pmpro--color--accent)" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg></span>
													<span class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field-password-toggle-state' ) ); ?>"><?php esc_html_e( 'הצג סיסמה', 'paid-memberships-pro' ); ?></span>
											</button>
									...
									<?php
										if ( $pmpro_checkout_confirm_password ) {
											...
											<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-password', 'pmpro_form_field-password2' ) ); ?>">
												<label for="password2" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('אשר סיסמה', 'paid-memberships-pro' );?></label>
									...
									<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-email pmpro_form_field-bemail', 'pmpro_form_field-bemail' ) ); ?>">
										<label for="bemail" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('כתובת אימייל', 'paid-memberships-pro' );?></label>
									...
									<?php
										if ( $pmpro_checkout_confirm_email ) {
											...
											<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-email pmpro_form_field-bconfirmemail', 'pmpro_form_field-bconfirmemail' ) ); ?>">
												<label for="bconfirmemail" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('אימות אימייל', 'paid-memberships-pro' );?></label>
						...
						<?php if ( ! $skip_account_fields ) { ?>
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_actions' ) ); ?>">
								<?php esc_html_e('כבר יש לך חשבון?', 'paid-memberships-pro' );?> <a href="<?php echo esc_url( wp_login_url( apply_filters( 'pmpro_checkout_login_redirect', pmpro_url("checkout", "?pmpro_level=" . $pmpro_level->id . $discount_code_link) ) ) ); ?>"><?php esc_html_e('התחבר כאן', 'paid-memberships-pro' ); ?></a>
							</div> <!-- end pmpro_card_actions -->
						<?php } ?>
					</div> <!-- end pmpro_card -->
			...
			<fieldset id="pmpro_billing_address_fields" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fieldset', 'pmpro_billing_address_fields' ) ); ?>" <?php if ( ! $pmpro_requirebilling || apply_filters("pmpro_hide_billing_address_fields", false) ) { ?>style="display: none;"<?php } ?>>
				<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card' ) ); ?>">
					<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
						<legend class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_legend' ) ); ?>">
							<h2 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_heading pmpro_font-large' ) ); ?>"><?php esc_html_e( 'כתובת לחיוב', 'paid-memberships-pro' ); ?></h2>
						</legend>
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fields pmpro_cols-2' ) ); ?>">
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-bfirstname', 'pmpro_form_field-bfirstname' ) ); ?>">
								<label for="bfirstname" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('שם פרטי', 'paid-memberships-pro' );?></label>
							...
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-blastname', 'pmpro_form_field-blastname' ) ); ?>">
								<label for="blastname" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('שם משפחה', 'paid-memberships-pro' );?></label>
							...
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-baddress1', 'pmpro_form_field-baddress1' ) ); ?>">
								<label for="baddress1" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('כתובת', 'paid-memberships-pro' );?></label>
							...
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-baddress2', 'pmpro_form_field-baddress2' ) ); ?>">
								<label for="baddress2" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('כתובת 2 (אופציונלי)', 'paid-memberships-pro' );?></label>
								...
								<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-bcity', 'pmpro_form_field-bcity' ) ); ?>">
									<label for="bcity" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('עיר', 'paid-memberships-pro' );?></label>
								...
								<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-bstate', 'pmpro_form_field-bstate' ) ); ?>">
									<label for="bstate" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('מחוז / איזור', 'paid-memberships-pro' );?></label>
								...
								<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-bzipcode', 'pmpro_form_field-bzipcode' ) ); ?>">
									<label for="bzipcode" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('מיקוד', 'paid-memberships-pro' );?></label>
							...
							<?php
								...
								if($show_country) { ?>
									<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-select pmpro_form_field-bcountry', 'pmpro_form_field-bcountry' ) ); ?>">
										<label for="bcountry" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('מדינה', 'paid-memberships-pro' );?></label>
							...
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_form_field-bphone', 'pmpro_form_field-bphone' ) ); ?>">
								<label for="bphone" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('טלפון', 'paid-memberships-pro' );?></label>
				...
				if ( $pmpro_include_payment_information_fields ) {
					...
					<fieldset id="pmpro_payment_information_fields" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fieldset', 'pmpro_payment_information_fields' ) ); ?>" <?php if ( ! $pmpro_requirebilling || apply_filters( 'pmpro_hide_payment_information_fields', false ) ) { ?>style="display: none;"<?php } ?>>
						<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card' ) ); ?>">
							<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_card_content' ) ); ?>">
								<legend class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_legend' ) ); ?>">
									<h2 class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_heading pmpro_font-large' ) ); ?>"><?php esc_html_e( 'פרטי תשלום', 'paid-memberships-pro' ); ?></h2>
								</legend>
								<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fields' ) ); ?>">
									...
									<?php if($pmpro_show_discount_code) { ?>
										<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_cols-2' ) ); ?>">
											<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_field pmpro_form_field-text pmpro_payment-discount-code', 'pmpro_payment-discount-code' ) ); ?>">
												<label for="pmpro_discount_code" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_label' ) ); ?>"><?php esc_html_e('קוד קופון', 'paid-memberships-pro' );?></label>
												<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_fields-inline' ) ); ?>">
													...
													<input aria-label="<?php esc_html_e( 'השתמש בקוד קופון', 'paid-memberships-pro' ); ?>" type="button" id="discount_code_button" name="discount_code_button" value="<?php esc_attr_e('החל', 'paid-memberships-pro' );?>" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_btn pmpro_btn-submit-discount-code', 'other_discount_code_button' ) ); ?>" />
												</div> <!-- end pmpro_form_fields-inline -->
			...
			<div class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_form_submit' ) ); ?>">
				...
				<?php if ( $pmpro_review ) { ?>
					<span id="pmpro_submit_span">
						...
						<input type="submit" id="pmpro_btn-submit" class="<?php echo esc_attr( pmpro_get_element_class( 'pmpro_btn pmpro_btn-submit-checkout', 'pmpro_btn-submit-checkout' ) ); ?>" value="<?php esc_attr_e('סיים תשלום', 'paid-memberships-pro' );?>" />
					</span>
				...
				<?php } else { ?>
					...
					<?php
						...
						if ( $pmpro_checkout_default_submit_button ) {
							...
							<span id="pmpro_submit_span">
								<input type="hidden" name="submit-checkout" value="1" />
								<input type="submit" id="pmpro_btn-submit" class="<?php echo esc_attr( pmpro_get_element_class(  'pmpro_btn pmpro_btn-submit-checkout', 'pmpro_btn-submit-checkout' ) ); ?>" value="<?php if($pmpro_requirebilling) { esc_html_e('השלם הזמנה', 'paid-memberships-pro' ); } else { esc_html_e('השלם הזמנה', 'paid-memberships-pro' );}?>" />
							</span>
							<?php
							}
						?>
				...
				<div id="pmpro_processing_message" style="visibility: hidden;">
					<?php
						$processing_message = apply_filters("pmpro_processing_message", __("אנא המתן", 'paid-memberships-pro' ));
